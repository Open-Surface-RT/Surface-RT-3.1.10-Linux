...
static int i2c_hid_open(struct hid_device *hid)
{
	...
	printk("i2c_hid: open start\n");

	mutex_lock(&i2c_hid_open_mut);
	if (!hid->open++) {
		printk("i2c_hid: open work\n");
		ret = i2c_hid_set_power(client, I2C_HID_PWR_ON);
		if (ret) {
			printk("i2c_hid: open fucked\n");
			hid->open--;
			goto done;
		}
			 ...
			 const struct i2c_device_id *dev_id)
{
	...
	printk("hid_add_device_status: %d\n", ret);
	if (ret) {
		if (ret != -ENODEV)
...
/////////////////////////////   Register Generic   /////////////////////////////
////////////////////////////////////////////////////////////////////////////////
static const struct hid_device_id hid_i2c_table[] = {
	{ HID_DEVICE(BUS_I2C, HID_ANY_ID, HID_ANY_ID) },
	{ }
};

static struct hid_driver hid_i2c_driver = {
	.name = "generic-i2c",
	.id_table = hid_i2c_table,
};
...
static int __init hid_init(void)
{
	...
	retval = hid_register_driver(&hid_i2c_driver);
	if (retval)
...
{
	hid_unregister_driver(&hid_i2c_driver);
}
